...
- img2img方法效果不理想
**解决历程**:
1. **第一次尝试**: 创建一致性生成策略 `lib/consistentGeneration.ts`
   - 先生成完整成品作为参考，确保风格统一
   - 问题：每步仍独立生成，没有真正的累积效果

2. **最终解决**: 创建渐进式生成策略 `lib/progressiveGeneration.ts`
   - 使用img2img技术，每一步基于前一步构建
   - 第一步：只画基础形状
   - 中间步骤：使用img2img在前一步基础上添加新元素
   - 最后一步：完成最终效果
   - 设置prompt_strength=0.7确保新元素被添加
   - 成功实现真正的逐步构建效果
